...
### 유사보어 설명
유사보어는 주어와 보어가 동일한 의미를 전달하지만, 보어가 주어의 상태나 특성을 설명하는 구조에서 주로 사용됩니다. 유사보어는 '보어'라는 역할을 하지만, 그 자체로 주어의 성질을 정의하거나 설명하는 역할을 합니다. 주격 보어는 주어를 설명하는 단어로, 보통 **형용사나 명사**로 이루어집니다. 

### 예시
- He was born poorly.  
  -> He was born poor.
- She returned home disappointedly.
  (ly는 부사, 즉 v수식 / 집에 돌아오는 행위가 실망스러워지는 것이 됨)
  ex) 아 오늘은 3회전 못하고 2회전 반 공중제비하면서 왔네 ㅠ
  -> She returned home disappointed.
   
일반적인 보어와 다르게 주격보어는 생략해도 문장 가능

### ✅ 문장 구조: [S + V (자동사) + 부사어 + 분사]

- **disappointed**는 주어 **She**의 상태를 설명하는 과거분사.
- 이는 **주격 보어(Subjective Complement)** 역할을 함.
- 즉, **자동사 + 분사(형용사)** 구조에서 주어의 상태를 보충 설명하는 **2형식 문장**.

🔸 완전한 문장 구조:  
→ She returned home. (완전함)  
→ disappointed는 없어도 문장은 성립 → **유사보어처럼 간주됨**
...
| V         | returned (자동사) |
| 부사어    | home (장소) |
| 주격 보어 | disappointed (주어 상태 설명하는 분사) |

→ 분사는 주어 상태를 설명하며 **보어처럼 작용**  
→ 시험·문법서에서는 **유사보어 예문**으로 분류되기도 함
